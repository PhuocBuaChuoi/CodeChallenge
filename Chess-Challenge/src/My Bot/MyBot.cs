...
{
    Dictionary<string,int> debugger = new Dictionary<string,int>();
    ...
    public int QuieSearch(Board board, int MdepSearch = 2)
    {
        int k = AnlBoard(board);//do nothing
        ...
        else
        {
            for (int i = 0; i < CaptureMove.Length; i++)
            {
                ...
                k = (!board.IsWhiteToMove) ? Math.Max(QuieSearch(board, MdepSearch - 1), k) : Math.Min(QuieSearch(board, MdepSearch - 1), k);
    ...
    public KeyValuePair<Move, int> Minimax(Board board, int depth = 0, int alpha = int.MinValue, int beta = int.MaxValue)
    {
        if (depth == 0)
        {
            ...
            //debugger.TryAdd(board.GetFenString(), d);
            return new KeyValuePair<Move, int>(move, d);
        }
        else
        {
            if (board.IsDraw()) { return new KeyValuePair<Move, int>(Move.NullMove, 0); }
    ...
    {
        KeyValuePair<Move, int> res = Minimax(board, 3);
        ...
        foreach(KeyValuePair<string,int> key in debugger)
        {
            //Console.Write(key.Key); Console.Write("-----    -----"); Console.WriteLine(key.Value);
        }
        return move;
    }
}